...
def get_execution_runs(client: DataHubGraph, ingestion_urn: str) -> List[str]:
    ...
    if num_entities == 0:
        logger.warning(f"No executions found for {ingestion_urn}")
        exit(0)
...
with Progress() as progress:
    ingestion_run_urns = [
        ingestion_run_urn.split(":")[-1]
        for ingestion_run_urn in get_execution_runs(datahub, ingestion)
    ]
    logger.info(f"Ingestion runs: {json.dumps(ingestion_run_urns)}")
    task = progress.add_task(
        "Processing execution requests...", total=len(ingestion_run_urns)
    )
    for ingestion_run_urn in ingestion_run_urns:
        progress.update(
            ...
            advance=1,
            description=str(
                f"Processing: urn:li:dataHubExecutionRequest:{ingestion_run_urn}"
            ),
        )

        # GraphQL approach to pull all entities that were updated by the ingestion run, not necessarily the key aspect
        urn_batches = batch_iter(
            datahub.get_urns_by_filter(
        ...
        for batch in urn_batches:
            ...
            for entity_type in urns_by_type.keys():
                if entity_type == "dataset":
                    response = datahub.get_entities_v2(
                        entity_name=entity_type,
                        urns=urns_by_type[entity_type],
                        aspects=["datasetKey"],
                    ...
                    for key in response:
                        if "systemMetadata" not in response[key]["datasetKey"]:
                            continue
                        if (
                            response[key]["datasetKey"]["systemMetadata"]["lastRunId"]
                            in ingestion_run_urns
                        ):
                            if dry_run:
                                logger.info(
                                    f"Would delete: {key} and {datahub.delete_references_to_urn(key, True)[0]} references"
                                )
                                urns_deleted = urns_deleted + 1
                            else:
                                datahub.delete_entity(key)
                                references_count, related_aspects = (
                                    datahub.delete_references_to_urn(key)
                                )
                                logger.info(
                                    f"Deleted: {key} and {references_count} references"
                                )
                                urns_deleted = urns_deleted + 1
                        else:
                            logger.debug(
                                f"Skipping {key} created by {response[key]['datasetKey']['systemMetadata']['lastRunId']}"
                            )
                else:

...
def sortByKeyAspectCreatedDate(urn: str, data: dict[str, dict[str, object]], index: DataHubIndex) -> int:
    ...
    if "time" not in created_metadata.keys():
        logger.error(f"Key aspect for {urn} does not have time defined in the created metadata")
        exit(0)
        return sys.maxsize
...
if __name__ == "__main__":
    with open(file_name) as f:
        ...
        sorted_urns = sorted(data.keys(), key=lambda k: sortByKeyAspectCreatedDate(k, data, index))

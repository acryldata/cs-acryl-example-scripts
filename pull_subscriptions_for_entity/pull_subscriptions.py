...
if num_entities == 0:
    logger.warning(f"No subscriptions found for {subscribed_urn}")
    exit(0)

urns: List[str] = [entry["entity"] for entry in results["value"]["entities"]]

for urn in urns:
    subscription: Optional[SubscriptionInfoClass] = client.get_aspect(
        entity_urn=urn, aspect_type=SubscriptionInfoClass
    )
    if not subscription:
        logger.warning(f"No subscription info found for urn: {urn}")

    subscribed_entity_change_types: List[EntityChangeDetailsClass] = (
        subscription.entityChangeTypes
    )

    if not subscribed_entity_change_types:
        logger.warning("No entity change types found")

    filtered_list = [
        change
        for change in subscribed_entity_change_types
        if change.entityChangeType in ["DEPRECATED", "OWNER_ADDED"]
    ]

    if len(filtered_list) > 0:
        print(
            f"Subscription: {urn} is listening for unintended changes: {subscribed_entity_change_types}"
        )

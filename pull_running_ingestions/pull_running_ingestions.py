...
urns = client.get_urns_by_filter(
    query="*", entity_types=["dataHubExecutionRequest"], extraFilters=filters
)

# Pull down the execution result aspect for each execution request that is currently running.
for urn in urns:
    execution_input: Optional[ExecutionRequestInputClass] = client.get_aspect(
        urn=urn, aspect_type=ExecutionRequestInputClass
    )
    type
    ...
    execution_request: Optional[ExecutionRequestResultClass] = client.get_aspect(
        urn=urn, aspect_type=ExecutionRequestResultClass
    )
